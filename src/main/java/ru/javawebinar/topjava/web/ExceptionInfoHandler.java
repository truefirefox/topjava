    ...
    public ErrorInfo conflict(HttpServletRequest req, DataIntegrityViolationException e) {
        ...
        if (rootMsg != null) {
            Optional<Map.Entry<String, String>> entry = constraintCodeMap.entrySet().stream()
                    ...
                    .findAny();
            if (entry.isPresent()) {
                String errorMsg = messageSource.getMessage(entry.get().getValue(), null, LocaleContextHolder.getLocale());
                return logAndGetErrorInfo(req, new DataIntegrityViolationException(errorMsg), false);
            }
        }
